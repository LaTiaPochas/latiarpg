...
const MAGIC_FOREST_OUTCOME_ITEM_IDS: Record<string, string> = {
  ...
  feather: "586746d5-4fa8-4cce-ac1d-06acc55b2c68",
...
export type NearWoodsGrantedItemView = {
  ...
  /** Cantidad otorgada en esta acción (p. ej. madera en bosque mágico puede ser > 1). */
...
export async function gatherNearWoods(): Promise<GatherNearWoodsResult> {
  ...
  const item = mapItemsRowToGrantedView(itemRow, 1);
...
export type GatherMagicForestResult = GatherNearWoodsResult;

export async function gatherMagicForest(): Promise<GatherMagicForestResult> {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  ...
  const rolled = rollMagicForestGatherOutcome();
  const { result, chance } = rolled;
  ...
  if (result === "enemy") {
    const encounterCode = rollMagicForestEnemyCombatCode();
    revalidatePath("/near-woods");
    revalidatePath("/character_profile");
    return {
      ok: true,
  ...
  const itemId = MAGIC_FOREST_OUTCOME_ITEM_IDS[result];
  if (!itemId) {
    return { ok: false, error: "Resultado sin ítem configurado." };
  }
